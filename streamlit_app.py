        ...
        st.plotly_chart(fig, use_container_width=True)
            ...
            for i in range(40, len(hist)):
                ...
                if not in_position:
                    ...
                    # Signal 1: Volume spike with price momentum
                    if hist['volume_ratio'].iloc[i] > 1.5 and current_price > hist['Close'].iloc[i-1]:
                        in_position = True
                        entry_price = current_price
                        entry_time = current_time
                        entry_reason = f"Volume spike (ratio: {hist['volume_ratio'].iloc[i]:.2f}) with bullish momentum"
                        signal_type = "SPY Long"
                        continue
                    
                    # Signal 2: Break above SMA with confirmation
                    if (current_price > hist['SMA_20'].iloc[i] and 
                        hist['Close'].iloc[i-1] <= hist['SMA_20'].iloc[i-1] and
                        hist['Volume'].iloc[i] > hist['volume_ma'].iloc[i]):
                        in_position = True
                        entry_price = current_price
                        entry_time = current_time
                        entry_reason = f"SMA breakout at ${hist['SMA_20'].iloc[i]:.2f} with volume confirmation"
                        signal_type = "SMA Breakout"
                        continue
                    
                    # Signal 3: Gap fill opportunity
                    if i > 0:
                        ...
                        if gap_pct > 0.003:  # 0.3% gap
                            ...
                            entry_price = current_price
                            entry_time = current_time
                            entry_reason = f"Gap detected ({gap_pct*100:.2f}%), anticipated mean reversion"
                            signal_type = "Gap Fill"
                            continue
                    ...
                    # Exit 1: Profit target (0.5% gain)
                    if pnl_pct >= 0.005:
                        exit_triggered = True
                        exit_reason = f"Profit target reached (+{pnl_pct*100:.2f}%)"
                    
                    # Exit 2: Stop loss (0.3% loss)
                    elif pnl_pct <= -0.003:
                        exit_triggered = True
                        exit_reason = f"Stop loss triggered ({pnl_pct*100:.2f}%)"
                    
                    # Exit 3: Time-based (hold for max 240 minutes = 4 hours)
                    elif time_in_trade >= 240:
                        exit_triggered = True
                        exit_reason = f"Time limit reached ({time_in_trade:.0f} min held)"
                    
                    # Exit 4: End of day exit (3:45 PM)
                    elif current_time.hour == 15 and current_time.minute >= 45:
                        exit_triggered = True
                        exit_reason = "End of day exit (3:45 PM)"
                    
                    # Exit 5: Reversal signal (price drops below entry with volume)
                    elif current_price < entry_price * 0.998 and hist['Volume'].iloc[i] > hist['volume_ma'].iloc[i] * 1.3:
                        exit_triggered = True
                        exit_reason = f"Reversal detected with volume spike"
                    ...
                    if exit_triggered:
                        ...
                        completed_trades.append({
                            ...
                            'Entry Reason': entry_reason,
                            'Exit Date': current_time.strftime('%m/%d/%Y'),
                            'Exit Time': current_time.strftime('%I:%M %p'),
                            'Exit Price': f"${current_price:.2f}",
                            'Exit Reason': exit_reason,
                            'Shares': shares,
                            'Hold Time (min)': f"{time_in_trade:.0f}",
                            'P&L': f"${pnl_dollars:.2f}",
                            'P&L %': f"{pnl_pct*100:.2f}%",
                            'Entry Price Numeric': entry_price,
                            'Exit Price Numeric': current_price,
            ...
            if not completed_trades:
                return pd.DataFrame(), "No completed trades found in historical data"
        ...
        for idx, trade in backtest_df.iterrows():
            ...
            with st.expander(f"Trade #{idx+1}: {trade['Signal Type']} - {trade['P&L']} ({trade['P&L %']})", expanded=False):
                ...
                with col2:
                    ...
                    st.write(f"🕐 Time: {trade['Exit Time']}")
                    st.write(f"💰 Price: {trade['Exit Price']}")
                    st.write(f"⏱️ Hold: {trade['Hold Time (min)']} min")
                    st.markdown(f"**Reason:** *{trade['Exit Reason']}*")
